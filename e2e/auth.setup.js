...
setup("authenticate as provider", async ({ page }) => {
  ...
  await expect(
    page.getByText("Test User").or(page.getByText("provider@example.com")),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Submit a crime form" }),
  ).toBeVisible();
...
setup("authenticate as supervisor", async ({ page }) => {
  ...
  // Wait until the page reaches a state where all cookies are set.
  await expect(
    page.getByLabel("Menu", { exact: true }).getByText("Super Visor"),
  ).toBeVisible();
...
setup("authenticate as caseworker", async ({ page }) => {
  ...
  // Wait until the page reaches a state where all cookies are set.
  await expect(
    page.getByLabel("Menu", { exact: true }).getByText("Case Worker"),
  ).toBeVisible();

...
test.describe('Non-Standard Magistrates original payment with amendment - As a Caseworker', () => {
    test('Creating a non-standard magistrates payment from scratch', async ({paymentsFixture}) => {
        ...
        await test.step('Create linked NSM amendment payment', async () => {
            ...
            await page.getByRole('link', { name: 'Create payment request' }).click();
            const claimTypePage = new ClaimTypePage(page);
            await claimTypePage.selectClaimType("Non-Standard Magistrates' - amendment");
            ...
            //Create payment
            expect(page.getByLabel('Date claim amended')).toBeVisible();
            await page.getByLabel('Date claim amended').fill(paymentData.nsmClaimDetails.dateAssessed);
